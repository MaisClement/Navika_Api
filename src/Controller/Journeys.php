...
class Journeys
{
    ...
    public function getJourneysSearch(Request $request)
    {
        ...
        if (!isset($from) || !isset($to)) {
            $this->logger->logHttpErrorMessage($request, 'At least one required parameter is missing or null, have you "from" and "to" ?', 'WARN');
            return new JsonResponse(Functions::httpErrorMessage(400, 'At least one required parameter is missing or null, have you "from" and "to" ?'), 400);
        }
    ...
    {
        $this->logger->log(['message' => "PRIM Journeys query: $url"], 'INFO');
        ...
        if ($status != 200) {
            $this->logger->log(["message" => "GFBS query: Unable to fetch data. HTTP error code $status"], 'ERROR');
