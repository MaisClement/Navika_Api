...
class Lines
{
    ...
    public function __construct(EntityManagerInterface $entityManager, RoutesRepository $routesRepository, Logger $logger, StopsRepository $stopsRepository, StopRouteRepository $stopRouteRepository, AgencyRepository $agencyRepository, ParameterBagInterface $params)
    {
    ...
    public function searchLines(Request $request): JsonResponse
    {
        ...
        if (is_string($query) && $query == "") {
            ...
            $this->logger->logHttpErrorMessage($request, 'At least one required parameter is missing or null, have you "q" ?', 'WARN');
            return new JsonResponse(Functions::httpErrorMessage(400, 'At least one required parameter is missing or null, have you "q" ?'), 400);
        }
    ...
    public function getLineSchedules($line_id, $stop_id, Request $request): JsonResponse
    {
        ...
        if (!Functions::isValidDateYMD($date)) {
            $this->logger->logHttpErrorMessage($request, 'Invalid date, please ensure date format is Y-m-d', 'WARN');
            return new JsonResponse(Functions::httpErrorMessage(400, 'Invalid date, please ensure date format is Y-m-d'), 400);
        }

        $routes = $this->stopRouteRepository->findOneBy(['stop_id' => $stop_id, 'route_id' => $line_id]);
        if ($routes == null) {
            $this->logger->logHttpErrorMessage($request, "Nothing where found for this route and stop", 'WARN');
            return new JsonResponse(Functions::httpErrorMessage(400, 'Nothing where found for this route and stop'), 400);
        }
        ...
        if (Functions::isToday($date) && str_contains($line_id, 'IDFM:')) {
            ...
            $this->logger->log(['message' => "PRIM Journeys query: $prim_url"], 'INFO');
            ...
            if ($status != 200) {
                $this->logger->logHttpErrorMessage($request, "PRIM Journeys query: Unable to fetch data. HTTP error code $status", 'WARN');
            ...
            foreach ($results as $result) {
                if (!isset($result->MonitoredVehicleJourney->MonitoredCall)) {
                    $this->logger->logHttpErrorMessage($request, "PRIM Journeys query: Unable to fetch data. Invalid content ", 'WARN');
                    return new JsonResponse(Functions::httpErrorMessage(520, 'Invalid fetched data'), 520);
                }
        ...
        $_terminus = Functions::getTerminusOfALine($db, $routes->getRouteId());
        ...
        $objs = Functions::getSchedulesByStop($db, $stop_id, $line_id, $date);
